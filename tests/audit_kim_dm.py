...
from warframe_lore.kim_dm import KimDM, WIKI_PAGE_MAP, parse_dialogue_file
...
class LocalMirrorAudit(unittest.TestCase):
    def test_all_native_graphs_against_raw_reachability_and_route_multiplicity(self):
        kim_dict = json.loads((MIRROR / "dicts" / "en.json").read_text(encoding="utf-8"))
        totals = Counter()
        node_types = Counter()
            ...
            with self.subTest(file=path.name):
                ...
                for nid, item in index.items():
                    ...
                    for output_index, output in enumerate(item.get("Outputs", [])):
                        ...
                        routes.append(("Outputs", output_index, output.get("Outgoing", []),
                ...
                for start in starts:
                    ...
                    for visible in graph["nodes"]:
                        ...
                        if visible["kind"] in ("npc", "choice"):
                            ...
                            expected_speaker = "Vous" if visible["kind"] == "choice" else (
                                kim_dict.get(speaker, speaker) or sender)
                            self.assertEqual(visible["speaker"], expected_speaker)
                        elif visible["kind"] == "chemistry":
                            self.assertEqual(visible["chemistryDelta"], item.get("ChemistryDelta", 0))
                        else:
                    ...
                    for line in conversation["messages"]:
                        self.assertIn((line["speaker"], line["text"], line["player"]), spoken)
                    for step in conversation["script"]:
                        self.assertIn(step["kind"], ("npc", "prompt"))
                        if step["kind"] == "npc":
                            self.assertIn((step["speaker"], step["text"], False), spoken)
                ...
                print(f"{path.name}: {len(conversations)} conversations, {len(raw)} native nodes")
        ...
        for wiki in WIKI_PAGE_MAP:
            ...
            self.assertEqual({n["id"]: n for n in graph["nodes"] if n["id"] != "root"},
                             expected_nodes)
            self.assertEqual({e["id"]: e for e in graph["edges"] if e["source"] != "root"},
                             expected_edges)
            self.assertEqual({e["target"] for e in graph["edges"] if e["source"] == "root"},
                             expected_roots)
