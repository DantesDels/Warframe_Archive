...
(dispatch, routing, streaming), ``member`` (accreditation, name resolution,
snapshots, card gate) and ``moderation`` (hostile sessions, répartie, answer
feedback).  Each mixin owns ONE concern and reads the bot's two attributes:
``state`` (:class:`BotState`) and ``services`` (:class:`BotServices`).
"""
