...
| `rag/` | `retriever.py` (`Retriever` contract + `RAGHit`), `search.py` (`CosinusSearch` pgvector), `structured_search.py` (`StructuredSearch`), `merged.py` (`MergedRetriever`), `prompt.py` (`PromptBuilder`), `service.py` (`RAGService`), `vocabulary.py` (`ArchiveVocabulary` — whole-corpus vocabulary behind the post-generation gate) |
