...
async def roleplay(websocket: WebSocket) -> None:
    ...
    try:
        ...
        while True:
            ...
            if payload.get("type") == FRAME_PERSONA:
            ...
            if payload.get("type") == FRAME_COMMENT:
                ...
                await websocket.send_json(
                    CommentFrame(text=comment).model_dump())
                continue
            if payload.get("type") != FRAME_MESSAGE:
                continue
            ...
            if detect_probe(user_text):
                await websocket.send_json(
                    TokenFrame(token=JAILBREAK_REJECT).model_dump())
                await websocket.send_json(
                    EndFrame(text=JAILBREAK_REJECT).model_dump())
                continue
            ...
            if not context_text and suggestion is None and want_rag:
                await websocket.send_json(
                    TokenFrame(token=RAG_ERROR).model_dump())
                await websocket.send_json(
                    EndFrame(text=RAG_ERROR).model_dump())
                continue
            ...
            if persona_mode == PERSONA_ORACLE and member_name:
                ...
                if member_roles is not None:
                    ...
                    organic_answer = external_organic_reply(
                        ...
                        reluctant=reluctant)
                await websocket.send_json(
                    TokenFrame(token=organic_answer).model_dump())
                await websocket.send_json(
                    EndFrame(text=organic_answer).model_dump())
                continue
            ...
            if persona_mode == PERSONA_ORACLE and is_identity_question(user_text):
            ...
            if identity_answer:
                await websocket.send_json(
                    TokenFrame(token=identity_answer).model_dump())
                await websocket.send_json(
                    EndFrame(text=identity_answer).model_dump())
                continue
